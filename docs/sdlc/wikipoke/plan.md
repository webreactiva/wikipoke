...
| FR-003 | delivered | Every page source carries `revision` and `hash`; `status` separates integrity findings, drift and uncovered scope; `lint` is structural only; relations carry `basis: observed \| inferred`; a changed source produces drift, not a falsehood verdict. | — |
...
| FR-007 | partial | `capture` persists an event, materializes a decision page and a task watchlog, keeps declared rationale and marks it unknown when absent. | Only a manual entry point. Nothing captures decisions automatically; the post-commit hook refreshes the attention signal and nothing else. |
...
| FR-013 | ⛔ superseded, delivered | `install` writes three agent-neutral skills and a notifier, composes a delegating Git hook only when none exists, and reports `activeHook` plus manual steps; `doctor` reports capabilities with or without a wiki; `uninstall` is reversible and preserves knowledge. | The unattended-execution clause is void. No scheduled invocation has been verified. |
