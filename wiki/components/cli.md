...
`countFindings` and `reportOne` each narrow it back (`src/bin/wikipoke.ts:260`). `CHECKS` survives
...
instead of reporting a missing checkpoint and missing index as errors (`src/bin/wikipoke.ts:246`) —
...
to install (`src/bin/wikipoke.ts:181`). Run by an agent or through a pipe there is nobody to
...
(`src/bin/wikipoke.ts:140`): what is missing, that without a hook nothing will ever say the wiki is
...
and since `aafa306` so does a re-run of `init` (`src/bin/wikipoke.ts:187`), which used to print
"no hook was installed" in a repository that had two, meaning *this run* installed none and reading
as *none is installed*.

`--dir` is checked before `init` writes anything (`src/bin/wikipoke.ts:170`). Until `b29f7f4` an
...
`hooks add` that updates them (`src/bin/wikipoke.ts:123`), in `init` and in bare `wikipoke hooks`
