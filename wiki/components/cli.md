...
stdout has drained (`src/bin/wikipoke.ts:330`). On a pipe stdout is written asynchronously, and
...
[the atlas](./atlas.md) (`src/bin/wikipoke.ts:234`); it never returns while serving, so it sits
...
`countFindings` and `reportOne` each narrow it back (`src/bin/wikipoke.ts:298`). `CHECKS` survives
...
`wikipoke init` first" (`src/bin/wikipoke.ts:235`), and a bad `--port` or an `--out` it refuses
...
instead of reporting a missing checkpoint and missing index as errors (`src/bin/wikipoke.ts:284`) —
...
(`src/bin/wikipoke.ts:191`). A person at a terminal gets the questions of `src/lib/setup.ts`, before
...
(`src/bin/wikipoke.ts:153`): what is missing, that without a hook nothing will ever say the wiki is
...
and since `aafa306` so does a re-run of `init` (`src/bin/wikipoke.ts:203`), which used to print
"no hook was installed" in a repository that had two, meaning *this run* installed none and reading
as *none is installed*.

`--dir` is checked before `init` writes anything (`src/bin/wikipoke.ts:183`). Until `b29f7f4` an
